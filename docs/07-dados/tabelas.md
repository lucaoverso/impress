...
| Pre-conselho | `pre_conselho_periodos` | PK `id`; UNIQUE `ano_letivo, etapa`. | `nome`, datas, `status`, datas de controle. | Confirmada pelo codigo: `database.py`: `criar_tabelas`. |
...
| Pre-conselho | `pre_conselho_registros` | PK `id`; FK periodo/disciplina/professor/turma/estudante; UNIQUE `professor_usuario_id, estudante_id, disciplina, ano_letivo, bimestre`. | Atencao, motivos JSON, observacoes, campos pos-preconselho, texto. | Confirmada pelo codigo: `database.py`: `criar_tabelas`. |
